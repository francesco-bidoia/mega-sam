...
def main():
    ...
    output_cvd_dir = source_path / 'outputs_cvd'
    run_cmd([
        'python', 'cvd_opt/cvd_opt.py',
        '--scene_name', scene_name,
        '--w_grad', '2.0',
        '--w_normal', '5.0',
        '--output_dir', str(output_cvd_dir)
    ], cwd=root_dir)
    ...
    cvd_file = output_cvd_dir / f'{scene_name}_sgd_cvd_hr.npz'
    if (cvd_file.exists() and cvd_file.parent != source_path) or not cvd_file.exists():
        root_cvd_file = root_dir / 'outputs_cvd' / f'{scene_name}_sgd_cvd_hr.npz'
        if root_cvd_file.exists():
            dst = source_path / 'outputs_cvd'
            dst.mkdir(parents=True, exist_ok=True)
            shutil.move(str(root_cvd_file), dst / root_cvd_file.name)
